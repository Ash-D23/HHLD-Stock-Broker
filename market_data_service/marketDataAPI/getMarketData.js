...
const connectWebSocket = async (wsUrl, instrumentKeys, callback) => {
 return new Promise((resolve, reject) => {
   ...
   ws.on("message", (data) => {
     callback(JSON.stringify(decodeProfobuf(data))); // Decode the protobuf message on receiving it
   });
...
const getMarketDataFeed = async (instrumentKeys, accessToken, callback) => {
